...
import higher
import random
from mixture_gaussian_ring import data_generator
import hypergrad as hg

plt.style.use('ggplot')
parser = argparse.ArgumentParser(description='Progressive Growing of GANs')

parser.add_argument('--method', default='LC', type=str,
                    help='')
...
parser.add_argument('--noise', default=0.01, type=float,
                    help='')
parser.add_argument('--r', default=5, type=float,
                    help='')
...
parser.add_argument('--lamda', default=0.005, type=float,
                    help='')
parser.add_argument('--gloop', default=10000, type=int,
                    help='')
parser.add_argument('--num_iter', default=5000, type=int,
                    help='')
parser.add_argument('--fixd', default=0.3, type=float,
                    help='')
parser.add_argument('--IV', action='store_true', default=False,
                    help='')
...
dset = data_generator(n=args.n, std=args.std, radius=args.r)
...
def plot_target(samples):
    ...
    plt.savefig('D:/Code_2023/GDC/LwCL_results_ring/'+prefix +'_'+ args.method + 'target' + '.png')
    plt.show()
    plt.close()
...
def plot_samples(samples, glist=None, KLlist=None, JSlist=None,step=0):
    ...
    for i, samps in enumerate(samples):
        ...
        plt.xticks([])
        plt.yticks([])
        plt.title('step %d' % (step))#i * log_interval
    ...
    plt.gcf().tight_layout()
    # plt.savefig(prefix + 'm{}_{}'.format(args.method, time.strftime("%Y_%m_%d_%H_%M_%S")) + '.png')
    plt.savefig('D:/Code_2023/GDC/LwCL_results_ring/' + prefix + 'm{}_step{}'.format(args.method, step) + '.png')
...
sample_points = sample_points0 + args.noise * np.random.randn(512, 2)
# scipy.io.savemat(
#     'target_{}_r{}_std{}.mat'.format(args.n, args.r, args.std),
#     mdict={'data': sample_points0})
scipy.io.savemat(
    'D:/Code_2023/GDC/LwCL_results_ring/target_m{}_{}_r{}_std{}.mat'.format(args.method,args.n, args.r, args.std),
    mdict={'data': sample_points0})
...
prefix = "W{}_unrolled_steps-{}_{}_zloop{}_yloop{}_zL2{}_y_L2{}_yln{}_ylr{}_xlr{}_-prior_std-{:.2f}".format(
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        if args.IV:
            iv0 = torch.std(x, 0)
            iv = torch.mean(iv0) * torch.ones_like(x[:, 0]).unsqueeze_(1)
            x = torch.cat((x, iv), 1)
        if args.method == 'WGAN':
...
def WGAN_loop_GP():
    ...
    if args.GN:
        ...
        for Fy, fy in zip(dFy, dfy):
            gFyfy = gFyfy + torch.sum(Fy * fy)
            gfyfy = gfyfy + torch.sum(fy * fy) + 1e-10
        # print(-gFyfy.detach() / gfyfy.detach())
        GN_loss = torch.clamp(-gFyfy.detach() / gfyfy.detach(),-0.1,0.1) * d_loss
        # GN_loss.backward(retain_graph=True)
        # D.load(backup)
        # del backup
        g_loss = g_loss + GN_loss
...
for it in range(num_iterations):#tqdm_notebook(range(num_iterations)):
    ...
    if args.method == 'BIM':
        ...
        g_info, d_loss, g_grad, d_grad, p, r, F1 = WGAN_loop_GP()
        ...
        timelist.append(time.time() - t0)
        # gradlist.append(g_grad)
    elif args.method == 'GAN' or 'SN':
        t0 = time.time()

        d_infos = [[0, 0], [0, 0]]
        # if not ( 0 != it % args.gloop and it>=args.fixd*num_iterations):
    ...
    if it % log_interval == 0:
        ...
        scipy.io.savemat(
            'D:/Code_2023/GDC/LwCL_results_ring/ring_result{}_GP{}_GN{}_lam{}_c{}_act{}_noise{}_n{}_std{}_us{}_glr{}_dlr{}_gloop{}_fixd{}.mat'.format(
                args.method, args.GP,
                args.GN, args.lamda,
                args.c,
                args.act, args.noise,
                ...
                args.std, args.us,
                args.glr,
                args.dlr, args.gloop, args.fixd),
            mdict={'data': samples, 'graddata': gradlist, 'gradGN': glist_GN, 'gloss': glist, 'dloss': dlist,
                   'time': timelist, 'p': plist, 'r': rlist,
                   'F1': F1list})
        # fidlist.append(fid.fea_fid(sample_points0, samples[-1]))
scipy.io.savemat(
    'D:/Code_2023/GDC/LwCL_results_ring/ring_result{}_GP{}_GN{}_lam{}_c{}_act{}_noise{}_n{}_std{}_us{}_glr{}_dlr{}_gloop{}_fixd{}_{}.mat'.format(args.method, args.GP,
                                                                                               args.GN, args.lamda,
                                                                                               args.c,
                                                                                               args.act, args.noise,
                                                                                               args.n,
