...
def fixed_point(params: List[Tensor],
    ...
    for k in range(K):
        ...
        if stochastic:
            w_mapped = fp_map(params, hparams)#f_step
...
def Darts_W_RHG(params_history: List[List[Tensor]],
                hparams: List[Tensor],
                K: int ,
                fp_map: Callable[[List[Tensor], List[Tensor]], List[Tensor]],
                outer_loss: Callable[[List[Tensor], List[Tensor]], Tensor],
                tol=1e-10,
                set_grad=True,
                stochastic=False,v0=-1,ita=0.9) -> List[Tensor]:
    params_history = [[w.detach().requires_grad_(True) for w in params] for params in params_history]
    params=params_history[-1]
    params = [w.detach().requires_grad_(True) for w in params]#y
    o_loss = outer_loss(params, hparams)#F
    grad_outer_w, grad_outer_hparams = get_outer_gradients(o_loss, params, hparams)#dy F,dx F
    w_mapped = fp_map(params, hparams,only_grad=True)#dy f
    t0=time.time()
    vs = [torch.zeros_like(w) for w in params] if v0==-1 else v0
    vsp =torch_grad(w_mapped, params, grad_outputs=vs, retain_graph=True,allow_unused=True)#dy (dy f) v=d2y f v
    vs = [ v0-ita*(v if v is not None else 0) + ita*(gow if gow is not None else 0) for v0,v, gow in zip(vs,vsp, grad_outer_w)]#(I-ita*d2yf)v+ita*dy F)
    params = params_history[-2]
    w_mapped = fp_map(params, hparams, only_grad=True)
    grads = torch_grad(w_mapped, hparams, grad_outputs=vs, allow_unused=True)#dx (dy f) v
    # time_W=time.time()-t0
    grads = [-g + v if g is not None else v for g, v in zip(grads, grad_outer_hparams)]
...
#     params = params_history[-2]
#     w_mapped = fp_map(params, hparams,only_grad=True)
#     w_mapped=[0.001 * (g if g is not None else 0) for w, g in zip(params, w_mapped)]
#     grads = grad_unused_zero(w_mapped, hparams, grad_outputs=grad_outer_w, retain_graph=True)#dx (dy f) v
#     # time_W=time.time()-t0
#     grads = [-g + v if g is not None else v for g, v in zip(grads, grad_outer_hparams)]
#
#     if set_grad:
...
# def Darts_W_RHG(params: List[Tensor],
#                 hparams: List[Tensor],
#                 K: int ,
#                 fp_map: Callable[[List[Tensor], List[Tensor]], List[Tensor]],
#                 outer_loss: Callable[[List[Tensor], List[Tensor]], Tensor],
...
def Darts_W_RHG_all(params: List[Tensor],
                hparams: List[Tensor],
                K: int ,
                fp_map: Callable[[List[Tensor], List[Tensor]], List[Tensor]],
                outer_loss: Callable[[List[Tensor], List[Tensor]], Tensor],
    ...
    vsp = torch_grad(w_mapped, params, grad_outputs=vs, retain_graph=True,allow_unused=True)#dy (dy f) v=d2y f v
    vs = [ v0-ita*(v if v is not None else 0) + ita*(gow if gow is not None else 0) for v0,v, gow in zip(vs,vsp, grad_outer_w)]#(I-ita*d2yf)v+ita*dy F)
    grads = torch_grad(w_mapped, hparams, grad_outputs=[torch.zeros_like(w) for w in params] if v0==-1 else v0, allow_unused=True)#dx (dy f) v
    time_W=time.time()-t0
    grads = [-g + v if g is not None else v for g, v in zip(grads, grad_outer_hparams)]
...
def Darts_W_CG(params: List[Tensor],
                hparams: List[Tensor],
                K: int ,
                fp_map: Callable[[List[Tensor], List[Tensor]], List[Tensor]],
                outer_loss: Callable[[List[Tensor], List[Tensor]], Tensor],
                tol=1e-10,
                set_grad=True,
                stochastic=False,v0=-1) -> List[Tensor]:
    ita=0
    ...
    grad_tem= torch_grad(w_mapped, params, grad_outputs=tem, retain_graph=True,allow_unused=True)#dy (dy f) v=d2y f v

    ita_l=list_tensor_matmul(tem,grad_tem,trans=1)
    # else:
    #     tras=list_tensor_matmul(w_mapped, vs, trans=1)
    #     vsp =[tras * w_map for w_map in w_mapped] # dy (dy f) v=d2y f v
    #     tem = [v - gow for v, gow in zip(vsp, grad_outer_w)]
    #
    #     ita_u = list_tensor_norm(tem) ** 2
 # dy (dy f) v=d2y f v

        # ita_l=list_tensor_matmul(tem,w_mapped,trans=1)**2



    # print(ita_u,ita_l)
    ita=ita_u/(ita_l+1e-12)
    # ita=torch.clamp(ita,0,1)
    # print(ita)
...
def Darts_W_CG_all(params: List[Tensor],
                hparams: List[Tensor],
                K: int ,
                fp_map: Callable[[List[Tensor], List[Tensor]], List[Tensor]],
                outer_loss: Callable[[List[Tensor], List[Tensor]], Tensor],
